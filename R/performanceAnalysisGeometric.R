...
# Analysis of performance of Geometric (in combination with BC approach)
source("./functions.R")
...
if(distortion == "sin"){
  ...
  results_geometric <- readRDS( 
    paste("./results/Results_geometric",
          "_ess_ORF_", ess_ORF,
          "_numLoci_", format(unique_loci, scientific = F),
          "_lambda_",lambda,
          "_bp_per_wave_", bp_per_wave , 
...
performance_sim <- lapply(seq_along(simulated_data), function(sim_run){
  ...
  sim_run_results <- results_geometric[[sim_run]]
  
  weights <- unique(sim_run_results$weighting)
  ...
  performance_per_weight <- lapply(weights, function(w){
    ...
    sim_run_prob_geometric <- 
      sim_run_results[round(sim_run_results$weighting, 2) == round(w, 2) , ]
    sim_run_prob_geometric$p_value[sim_run_prob_geometric$p_value==Inf] <- 1
    sim_run_prob_geometric$p_value[sim_run_prob_geometric$p_value==-Inf] <- 0
    
    
    pvalues <- unlist(sim_run_prob_geometric$p_value)
...
if(distortion == "sin"){
  saveRDS(performance_sim, paste("./performance/Performance_Geometric",
                                 "_ess_ORF_", ess_ORF,
                                 "_numLoci_", format(unique_loci, scientific = F),
                                 "_lambda_",lambda,
                                 "_bp_per_wave_", bp_per_wave , 

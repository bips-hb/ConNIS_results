...
results_RPplusV <- ConNIS(
  ...
  gene.stops    = gene_list$end,
  genome.length = 4109437,
  weight        = 0.75#selected_w_RPplusV
)
...
results_RPplusM<- ConNIS(
  ...
  gene.stops    = gene_list$end,
  genome.length = 4109437,
  weight        = 0.75#selected_w_RPplusM
)
...
results_RPplusVandM <- ConNIS(
  ...
  gene.stops    = gene_list$end,
  genome.length = 4109437,
  weight        = 0.75#selected_w_RPplusVandM
)
...
fitness_data_connis <-
  ...
  mutate(essentiality = case_when(
    -log(p_initial,10) >= -log(0.05/nrow(fitness_data_connis),10) &
      -log(p_RP,10) >= -log(0.05/nrow(fitness_data_connis),10) ~ "essential",
    -log(p_initial,10) < -log(0.05/nrow(fitness_data_connis),10) &
      -log(p_RP,10) >= -log(0.05/nrow(fitness_data_connis),10) ~ "quasi-essential",
    -log(p_initial,10) < -log(0.05/nrow(fitness_data_connis),10) &
      -log(p_RP,10) < -log(0.05/nrow(fitness_data_connis),10) ~ "non-essential",
    -log(p_initial,10) < -log(0.05/nrow(fitness_data_connis),10) &
      -log(p_RP,10) >= -log(0.05/nrow(fitness_data_connis),10) ~ "possible artifact"
  ))

fitness_data_connis$essentiality[is.na(fitness_data_connis$essentiality)] <- "possible artifact"

ggplot(fitness_data_connis,
       aes(x=log(-log(p_initial,10)), y=log(-log(p_RP,10)), color=essentiality)) +
  geom_point() +
  geom_hline(yintercept = log(-log(0.05/nrow(fitness_data_connis),10)), color="lightgrey")+
  geom_vline(xintercept = log(-log(0.05/nrow(fitness_data_connis),10)), color="lightgrey")+
...
genes_for_comparison <-
  intersect(fitness_data_connis$Gene,
            fitness_data_original$Gene)

fitness_data_connis_reduced <-
  fitness_data_connis %>% filter(Gene %in% genes_for_comparison)

fitness_data_original_reduced <-
  fitness_data_original %>% filter(Gene %in% genes_for_comparison)
